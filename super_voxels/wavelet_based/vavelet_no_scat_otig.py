...
def one_time_filter( coeffs, thresh_val, verbose=False):
    """ 
    so it get coefficients and threshold them so seem that zero out all of the 
    wavelets with coefficients that will not be above threshold
    """
...
def recursive_filter(img_var, coeffs, verbose=False):
    """ 
    so above we had fixed the threshold here seem that we are trying recursively tto find the best 
    threshold
    """
    noise_var = img_var # so image variance is set as a noise variance - seem to big
    ...
    niter = 0
    # seem that we are looping until the change between iterations begins to be insignificant
    while (np.abs(thresh_new-thresh_old) > rel_tol*thresh_old):
        thresh_old = thresh_new
        # I suppose it is about just getting  the way to choose the threshold to get the best possible 
