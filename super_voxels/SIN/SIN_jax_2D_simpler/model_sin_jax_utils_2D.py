...
#   mask = mask.reshape([logits.shape[0], 1])
  loss = -jnp.sum(one_hot_labels * log_softmax_logits * mask) / (mask.sum()+epsilon )
...
def get_checkerboard_arr(shift_x:float,shift_y:float,mask_shape:Tuple[int],epsilon:float):
    w=mask_shape[1]//2
    h=mask_shape[0]//2
    # shift_x=shift_x-epsilon
    # shift_y=shift_y-epsilon
    re = jnp.r_[ w*[shift_x ,1-shift_x] ]              
    ro = jnp.r_[ w*[shift_y ,1-shift_y] ]              
    return jnp.row_stack(h*(re, ro))    

def translate_mask_in_axis(mask:jnp.ndarray,shift_x:int,shift_y:int,mask_shape:Tuple[int],epsilon):
    checkerBoard_arr = get_checkerboard_arr(shift_x,shift_y,mask_shape,epsilon)
    return jnp.multiply(checkerBoard_arr,mask)
...
def get_translated_mask_variance(image:jnp.ndarray
    ...
    features=jnp.stack([
        get_image_features(image,translate_mask_in_axis(mask,epsilon,epsilon,mask_shape,epsilon),epsilon),
        get_image_features(image,translate_mask_in_axis(mask,epsilon,1-epsilon,mask_shape,epsilon),epsilon),
        get_image_features(image,translate_mask_in_axis(mask,1-epsilon,epsilon,mask_shape,epsilon),epsilon),
        get_image_features(image,translate_mask_in_axis(mask,1-epsilon,1-epsilon,mask_shape,epsilon),epsilon)
              ])
    # maxes= jnp.max(features,axis=0)
    # features=features/maxes
    feature_variance=jnp.var(features,axis=0)*feature_loss_multiplier
    #the bigger the supervoxel the bigger the variance is acceptable
    return jnp.mean(feature_variance)#*(1/ ((jnp.sum(mask.flatten())*10) + epsilon))
...
def get_edge_loss(mask_new_bi_channel,edge_forward_diff,edge_back_diff,epsilon,cfg):        
    ...
    edge_forward_diff=jnp.stack([edge_forward_diff_min,edge_forward_diff]  ,axis=-1)
    edge_back_diff=jnp.stack([edge_back_diff_min,edge_back_diff]  ,axis=-1)
...
class Apply_on_single_area(nn.Module):
    ...
    def get_feature_var_loss(self,resized_image,mask_combined,mask_combined_alt,epsilon, initial_mask_id):
        ...
        #local normalization
        resized_image= resized_image- jnp.min(resized_image.flatten())
        resized_image= resized_image/jnp.max(resized_image.flatten())
...
def get_edge_diffs(resized_image,dim_stride_curr,edge_map_end,epsilon,un_rearrange_to_intertwine_einops,cfg):
    ...
    to_end_grid=jnp.zeros(tuple(edge_map_end))
    ...
    return edge_map,edge_forward_diff,edge_back_diff
...
class De_conv_batched_multimasks(nn.Module):
    ...
    def __call__(self, image:jnp.ndarray, mask_old:jnp.ndarray,deconv_multi:jnp.ndarray,initial_masks:jnp.ndarray) -> jnp.ndarray:
        ...
        edge_map,edge_forward_diff,edge_back_diff=get_edge_diffs(resized_image
                                                        ,self.dim_stride_curr
                                                        ,self.edge_map_end
                                                        ,self.cfg.epsilon
                                                        ,self.un_rearrange_to_intertwine_einops
                                                        ,self.cfg)
