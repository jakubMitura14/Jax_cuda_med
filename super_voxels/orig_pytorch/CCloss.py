...
class CCLoss(nn.Module):
    ...
    Basically we just perform summations using convolutions where filter is just matrix of ones
    As additional implementation detail we are making sure that the calculated values are not less than 0 uing those maximum functions
    one input is sobel filtered image other algorithm output if I get it right 
    """
